<template>
   <section
      ...
      <template v-if="cart.length">
         <div class="w-[100%] mb-[30px] flex items-center justify-between">
            <div class="flex items-center font-bold">
               <img class="w-[29px] h-[29px] fill-myblack mr-[17px]"
                  src="@/assets/cartBlack.svg" />
               <h1 class="md:text-e leading-[39px] text-d text-myblack">
                  Basket
               </h1>
            </div>
            <button class="flex" @click="clearCart">
               <img class="h-[20px] w-[20px]" src="@/assets/trash.svg"
                  alt="trash" />
         ...
         <div class="w-full">
            <div
               ...
               <span>
                  Total product:
                  <span class="font-extrabold">{{ quantifyProd }} pieces.</span>
               </span>
               <span class="text-textSilver">
                  Order amount:
                  <span class="text-mypink font-bold">{{ totalCart }} $</span>
               </span>
            </div>
   ...
   <!-- <model-order :toggle="toggle" v-if="toggle" /> -->
</template>
